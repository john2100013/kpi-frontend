...
const SMSConfiguration: React.FC = () => {
  ...
  const fetchAllConfigs = async (companiesList: Company[]) => {
    ...
    await Promise.all(
      companiesList.map(async (company) => {
        try {
          const response = await api.get('/sms-settings/sms-config', {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      const response = await api.post('/sms-settings/sms-config', formData, {
        params: { companyId: selectedCompanyId }
      });
  ...
  const handleToggleStatus = async () => {
    ...
    try {
      ...
      const response = await api.put('/sms-settings/sms-config/toggle', {
        is_active: newStatus
      }, {
        params: { companyId: selectedCompanyId }
      });
  ...
  const handleTestSMS = async () => {
    ...
    try {
      setTesting(true);
      const response = await api.post('/sms-settings/sms-config/test', {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await api.delete('/sms-settings/sms-config', {
        params: { companyId: selectedCompanyId }
      });
